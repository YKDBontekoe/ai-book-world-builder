...
vi.mock("@/components/atoms/context-menu", () => ({
  ...
  ContextMenuTrigger: ({ children }: any) => <div data-testid="ctx-trigger">{children}</div>,
  ContextMenuContent: ({ children }: any) => <div data-testid="ctx-content">{children}</div>,
...
vi.mock("@/features/writer/components/left-sidebar/scene-item", () => ({
  SceneItem: ({ scene, onSelect, onRename, onDelete, isSelected, onToggleSelect }: any) => (
...
describe("SidebarChapter", () => {
  ...
  it("handles renaming chapter via context menu", async () => {
    ...
    expect(defaultProps.onRenameChapter).toHaveBeenCalledWith("c1", "New Chapter Name");
  });
