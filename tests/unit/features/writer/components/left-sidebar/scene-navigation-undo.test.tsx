...
vi.mock("@/features/writer/components/left-sidebar/scene-item", () => ({
	SceneItem: ({ scene, onDelete, onToggleSelect }: any) => (
		<div data-testid={`scene-item-${scene.id}`}>
            {scene.title}
            <button type="button" onClick={() => onDelete(scene.id)} aria-label={`Delete ${scene.title}`}>Delete</button>
            <button type="button" onClick={() => onToggleSelect(scene.id)} aria-label={`Select ${scene.title}`}>Select</button>
        </div>
	),
}));
...
describe("SceneNavigation Undo Delete", () => {
    ...
    it("optimistically removes scene", async () => {
        ...
        const deleteButton = screen.getByRole("button", { name: /delete scene 1/i });
