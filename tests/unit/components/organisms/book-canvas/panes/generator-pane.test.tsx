...
describe("GeneratorPane", () => {
	...
	it("renders the initial form state", () => {
		...
		expect(
			screen.getByLabelText(/What is your story about?/i),
		).toBeInTheDocument();
	...
	it("calls generateBookPlan when form is submitted", async () => {
		const mockPlan = {
			...
			chapters: [{ title: "Chapter 1", summary: "Summary 1" }],
		};
	...
	it("displays error when generation rejects (network error)", async () => {
		vi.mocked(StoryGenerationActions.generateBookPlan).mockRejectedValue(
