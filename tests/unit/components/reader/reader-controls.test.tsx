...
describe('ReaderControls', () => {
  ...
  it('calls onPreviousChapter when prev button is clicked', () => {
    ...
    const prevButtons = screen.getAllByRole('button').filter(b => b.textContent?.includes('Prev') || b.innerHTML.includes('ArrowLeft'));
    ...
    const prevNav = screen.getByText((content, element) => content.includes('Prev') && element?.tagName === 'BUTTON');
