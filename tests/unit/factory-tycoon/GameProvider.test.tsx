...
const TestComponent = () => {
  const { state, addBuilding, sellResource, manualInteract, forceSave, isRunning } = useGame();
  ...
  return (
    <div>
      ...
      <button onClick={() => addBuilding('Mine', 0, 0)}>Add Mine</button>
      <button onClick={() => sellResource('ore')}>Sell Ore</button>
      <button onClick={() => manualInteract(0, 0)}>Interact</button>
      <button onClick={() => forceSave()}>Save</button>
    </div>
  );
};
...
describe('GameProvider Integration', () => {
  ...
  beforeEach(() => {
    mockLoad.mockReset();
    mockSave.mockReset();
    mockLoad.mockResolvedValue({ success: true, data: { ...INITIAL_STATE, cash: 500 } });
  ...
  it('should handle load error', async () => {
    ...
    await waitFor(() => {
        expect(screen.getByTestId('cash')).toHaveTextContent(INITIAL_STATE.cash.toString());
        expect(screen.getByTestId('is-running')).toHaveTextContent('true');
    });
  });
