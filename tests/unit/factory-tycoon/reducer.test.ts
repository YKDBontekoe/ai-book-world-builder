...
import { type Action, gameReducer } from "@/features/factory-tycoon/reducer";
import type { GameState } from "@/features/factory-tycoon/types";

// Mock dependencies
vi.mock("@/features/factory-tycoon/engine", () => ({
	simulateTick: vi.fn((state: GameState) => ({
		...state,
		tickCount: state.tickCount + 1,
	})),
	calculateCapacity: vi.fn((): number => 100),
}));

vi.mock("@/features/factory-tycoon/systems/interactionSystem", () => ({
	processInteraction: vi.fn((state: GameState) => ({
		...state,
		cash: state.cash + 1,
	})),
}));
...
describe("gameReducer", () => {
	...
	beforeEach(() => {
		state = { ...JSON.parse(JSON.stringify(INITIAL_STATE)), cash: 1000 }; // Start with plenty of cash
	});
	...
	it("should handle ADD_BUILDING", () => {
		...
		expect(newState.buildings[0]).toEqual(
