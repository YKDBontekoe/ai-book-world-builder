...
describe("getProjectPreviewData", () => {
	...
	it("should return preview data on success", async () => {
		mocks.auth.mockResolvedValue({ user: { id: "user-1" } });
		mocks.projectRepository.findByIdWithAccess.mockResolvedValue({
			id: "project-1",
		});
		...
		const whereMock1 = vi
			.fn()
			.mockResolvedValue([{ sceneCount: 10, wordCount: 500 }]);
		...
		const selectBuilderChapter = {
			from: vi.fn().mockReturnValue(fromMockChapter),
		};
