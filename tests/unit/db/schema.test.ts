...
describe("db schema barrel", () => {
  ...
  it("retains source material processing indexes", () => {
    const processingIndexes = getTableConfig(sourceMaterialProcessing).indexes.map(
  ...
  it("preserves outline sequencing guarantees", () => {
    const chapterIndexes = getTableConfig(chapter).indexes.map(
      (idx) => ({ name: idx.config.name, unique: idx.config.unique })
    );
  ...
  it("exposes export and template indexes", () => {
    ...
    expect(exportIndexes).toEqual(
      expect.arrayContaining(["book_export_project_idx", "book_export_user_idx"])
    );
  ...
  it("continues to provide typed exports via the barrel", () => {
    ...
    expect(Object.keys(legacySchema)).toEqual(
      expect.arrayContaining(["messageDeprecated", "voteDeprecated", "legacySchemas"])
    );
  });
});
