...
function buildMessage({ chatId, id }: { chatId: string; id: string }): DBMessage {
  return {
    attachments: [],
    chatId,
    createdAt: new Date("2024-01-01T00:00:00.000Z"),
...
describe("deleteTrailingMessages", () => {
  ...
  it("throws when the chat does not belong to the session user", async () => {
    ...
    mockedGetChatById.mockResolvedValue(buildChat({ id: "chat-1", userId: "user-2" }));
  ...
  it("deletes trailing messages for the chat owner", async () => {
    ...
    mockedGetChatById.mockResolvedValue(buildChat({ id: "chat-1", userId: "user-1" }));
