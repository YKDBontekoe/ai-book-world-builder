...
describe('ConsistencyService', () => {
  ...
  it('should analyze project and create issues', async () => {
    ...
    mocks.getEntitiesForProject.mockResolvedValue([
      { id: 'e1', name: 'Alice', kind: 'character', summary: 'A brave warrior.' }
    ]);
    mocks.getScenesForProject.mockResolvedValue([
      { id: 's1', title: 'Scene 1', content: 'Alice runs away.', status: 'drafted' }
    ]);
    ...
    // Verify
    expect(mocks.getEntitiesForProject).toHaveBeenCalledWith({ projectId: 'p1' });
