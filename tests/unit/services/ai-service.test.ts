...
describe("AIService", () => {
	...
	describe("batchWriteChapter", () => {
		it("should iterate scenes and draft content", async () => {
			...
			mocks.generationService.draftScene.mockResolvedValue({
	...
	describe("rewriteScene", () => {
		it("should generate rewrite text", async () => {
			// Setup
			mocks.db.query.scene.findFirst.mockResolvedValue({
				id: "s1",
				title: "Scene 1",
				content: "Old",
				projectId: "p1",
			});
			mocks.generationService.continueWriting.mockResolvedValue({
				text: "New content",
			});
