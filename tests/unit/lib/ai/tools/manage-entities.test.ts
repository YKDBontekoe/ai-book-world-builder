...
describe("manageEntities tool", () => {
	...
	it("should FAIL to create entity when user does NOT own project", async () => {
		...
		const tool = manageEntities({
			session: mockOtherUserSession as any,
			projectId,
		}) as any;
		...
		if (result.error) {
			...
			if (firstResult?.success) {
				throw new Error(
					"VULNERABILITY DETECTED: Created entity in project not owned by user",
				);
			}
	...
	it("should FAIL to update entity when user does NOT own project", async () => {
		...
		if (firstResult?.success) {
			throw new Error(
				"VULNERABILITY DETECTED: Updated entity not owned by user",
			);
		}
