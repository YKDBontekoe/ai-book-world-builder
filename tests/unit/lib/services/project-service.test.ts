...
describe('ProjectService', () => {
  ...
  describe('deleteProjects', () => {
    ...
    it('should return error if projectIds length is greater than 50', async () => {
      ...
      expect(result).toEqual({ error: 'Cannot delete more than 50 projects at once.' });
    });
    ...
    it('should successfully delete owned projects and related data', async () => {
      ...
      (db.select as any).mockReturnValue(createMockQB([{ id: 'proj-1', userId: userId }]));
  ...
  describe('forkProject', () => {
    ...
    it('should return error if project is too large (entities + scenes > 2000)', async () => {
      ...
      const result = await projectService.forkProject(originalProjectId, userId);
      expect(result).toEqual({
        error: 'Project is too large to fork instantly. Please export and import instead.',
      });
    });
    ...
    it('should successfully fork a project', async () => {
      ...
      (projectRepository.findByIdWithAccess as any).mockResolvedValue(mockProject);
      ...
      (db.transaction as any).mockImplementation(async (cb: any) => {
        ...
        mockTx.select = vi.fn().mockImplementation(() => {
             ...
             if (callCount % 2 === 1) {
                 ...
                 return createMockQB([{ id: 'item-1', projectId: originalProjectId }]);
             } else {
      ...
      const result = await projectService.forkProject(originalProjectId, userId, 'New Name');
    ...
    it('should handle errors during fork', async () => {
       (db.$count as any).mockResolvedValue(10);
       (projectRepository.findByIdWithAccess as any).mockResolvedValue({ id: 'p1' });
