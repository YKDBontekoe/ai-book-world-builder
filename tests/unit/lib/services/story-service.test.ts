...
const mocks = vi.hoisted(() => ({
    ...
    limit: vi.fn().mockResolvedValue([{ id: "mock-id", sequence: 1, content: "mock content", chapterId: "mock-chapter-id", projectId: "mock-project-id", notes: "mock notes", title: "mock title" }]),
...
describe("StoryService", () => {
    ...
    describe("generateBookPlan", () => {
        it("should call generateObject with correct parameters", async () => {
            ...
            expect(result).toEqual({ title: "Test Book", summary: "Summary", chapters: [] });
        });
    });
    ...
    describe("createBookFromPlan", () => {
        it("should execute transaction and insert records", async () => {
            ...
            expect(mocks.ensureProjectAccess).toHaveBeenCalledWith("project-id", true);
