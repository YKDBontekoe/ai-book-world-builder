...
export class WriterPage {
  ...
  async createChapter(expectedTitle: string) {
    ...
    const addChapterBtn = this.page.getByRole("button", { name: "Add Chapter" }).first();
  ...
  async createScene(chapterTitle: string, sceneTitle: string) {
      ...
      const chapterItem = this.page.getByRole("button", { name: chapterTitle }).first();
      ...
      const chapterContainer = this.page.locator("div.group", { has: this.page.getByText(chapterTitle) }).first();
      ...
      await this.page.getByRole("menuitem", { name: "Generate Scenes (AI)" }).click();
      ...
      await expect(this.page.getByText("Scene 1")).toBeVisible({ timeout: 10000 });
  }
