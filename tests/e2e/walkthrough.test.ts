...
test.describe("App Walkthrough", () => {
  ...
  test("Full User Journey: Signup -> Project -> Writer -> Chat -> Delete", async ({ page }) => {
    ...
    await expect(writer.editor).toContainText("It was a dark and stormy night.");
    ...
    const assistantToggle = page.getByRole("button", { name: /assistant|chat/i });
    ...
    // Find project card
    const projectCard = page.locator("div").filter({ hasText: projectName }).last();
    // Look for menu trigger
    const menuTrigger = projectCard.getByRole("button").filter({ has: page.locator("svg") }).first(); // Approximate
