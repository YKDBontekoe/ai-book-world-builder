...
export function EntityInsights({ stats }: { stats: EntityStats }) {
    const data = Object.entries(stats.byKind).map(([name, value]) => ({ name, value }));
                         ...
                         {stats.mostConnected.length > 0 ? (
                             <ul className="space-y-3">
                                {stats.mostConnected.map((entity, i) => (
                                    <li key={entity.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 transition-colors">
                                        <div className="flex items-center gap-3">
                                            <span className="text-muted-foreground font-mono text-xs w-4">#{i + 1}</span>
                                            <div>
                                                <div className="font-medium text-sm">{entity.name}</div>
                                                <div className="text-[10px] text-muted-foreground uppercase tracking-wider">{entity.kind}</div>
                                            </div>
                                        </div>
                                        <span className="font-bold text-sm bg-primary/10 text-primary px-2 py-1 rounded-md">{entity.connections} rels</span>
                                    </li>
                                ))}
                             </ul>
                         ) : (
