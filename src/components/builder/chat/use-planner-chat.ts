    ...
    useEffect(() => {
        if (data?.messages) {
            const mappedMessages: Message[] = data.messages.map((m: any) => {
                ...
                const toolCall = m.parts?.find((p: any) => p.type === "tool-invocation" && p.toolName === "propose_plan");
                if (toolCall) {
    ...
    const { mutate: sendMessage, isPending: isSending } = useMutation({
        ...
        onSuccess: (result, _vars, context) => {
            ...
            setMessages(prev => {
                ...
                return prev.map(m => m.id === context?.tempId ? { ...m, isOptimistic: false } : m).concat(assistantMsg);
            });
