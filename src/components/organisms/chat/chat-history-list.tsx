...
export function ChatHistoryList({ projectId, onSelectChat, currentChatId }: ChatHistoryListProps) {
    ...
    const {
        ...
        queryFn: async ({ pageParam }) => {
            const params: Record<string, string | number> = { limit: PAGE_SIZE, projectId };
            if (pageParam) {
    ...
    const { mutate: deleteChat } = useMutation({
        ...
        onMutate: async (chatId) => {
            ...
            const previousHistory = queryClient.getQueryData<InfiniteData<ChatHistory>>(QUERY_KEYS.chatHistory());
        ...
        onError: (error, _, context) => {
            ...
            if (context?.previousHistory) {
                queryClient.setQueryData(QUERY_KEYS.chatHistory(), context.previousHistory);
            }
        },
    });
                ...
                <div className="flex flex-col gap-6 p-4">
                    {Object.entries(groupedChats).map(([group, chats]) => {
                        ...
                        switch (group) {
                            ...
                            case "lastWeek": label = "Last 7 Days"; break;
                            case "lastMonth": label = "Last 30 Days"; break;
                            case "older": label = "Older"; break;
                        }

                        return (
                            <div key={group} className="flex flex-col gap-2">
                                <h3 className="text-xs font-medium text-muted-foreground px-2">{label}</h3>
                                {chats.map((chat) => (
                                    <div
                                        key={chat.id}
                                        className={cn(
