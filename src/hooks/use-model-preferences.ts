...
export function useModelPreferences(
	...
	const [preferences, setPreferences] =
		useState<ModelPreferencesState>(DEFAULT_PREFERENCES);
		...
		(modelId: string, direction: "up" | "down") => {
			...
			startTransition(async () => {
				try {
					...
					setError(
						err instanceof Error ? err.message : "Unable to reorder favorites.",
					);
					setPreferences((current) => ({
						...current,
	...
	const recordRecentModel = useCallback(
		(modelId: string) => {
			const filteredRecent = preferences.recentModels.filter(
				(id) => id !== modelId,
			);
