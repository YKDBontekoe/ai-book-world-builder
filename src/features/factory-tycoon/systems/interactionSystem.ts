...
export function getInteractionResult(state: GameState, x: number, y: number): InteractionResult {
    ...
    if (building.localInventory && config.outputs) {
        ...
        if (outputRes && building.localInventory[outputRes] && (building.localInventory[outputRes] || 0) > 0) {
...
export function processInteraction(state: GameState, x: number, y: number): GameState {
    const buildingIndex = state.buildings.findIndex(b => b.x === x && b.y === y);
