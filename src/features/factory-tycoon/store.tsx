	...
	switch (action.type) {
		...
		case "MANUAL_INTERACT": {
			...
			if (building.localInventory && config.outputs) {
				...
				const currentAmount = building.localInventory[outputRes];
				if (outputRes && currentAmount && currentAmount > 0) {
		...
		(x: number, y: number) => {
			...
			if (b) {
				...
				// Machine Feedback
					...
					const outputRes = Object.keys(config.outputs)[0] as Resource;
					const currentAmount = b.localInventory[outputRes];
					if (outputRes && currentAmount && currentAmount > 0) {
						toast.success(`Collected ${currentAmount} ${outputRes}`);
					}
				}
			}
