...
export function GameCanvas({
	...
	// STABLE HANDLERS PATTERN
	// We use refs to hold the latest version of the handler logic,
	// and expose a stable function to GameTile.

	const handleTileClickLogic = (x: number, y: number) => {
		if (selectedBuilding) {
	...
	const handleTileClickRef = useRef(handleTileClickLogic);
	useEffect(() => {
		handleTileClickRef.current = handleTileClickLogic;
	});
	const onTileClick = useCallback(
		(x: number, y: number) => handleTileClickRef.current(x, y),
		[],
	);

	const handleContextMenuLogic = (e: React.MouseEvent, id?: string) => {
		e.preventDefault();
		if (id) {
			removeBuilding(id);
			playSound("delete");
	...
	const handleContextMenuRef = useRef(handleContextMenuLogic);
	useEffect(() => {
		handleContextMenuRef.current = handleContextMenuLogic;
	});
	const onContextMenu = useCallback(
		(e: React.MouseEvent, id?: string) => handleContextMenuRef.current(e, id),
		[],
	);

	const onMouseEnter = useCallback((x: number, y: number) => {
		setHoveredTile({ x, y });
	}, []); // setHoveredTile is stable

	// Create a map for O(1) lookup - MEMOIZED
