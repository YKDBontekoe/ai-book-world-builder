...
export function BuildingPalette({
	...
	const unlockedByCategory = Object.entries(BUILDING_CATEGORIES)
		.map(([category, types]) => ({
			category,
			buildings: (types as readonly BuildingType[]).filter((type) =>
				state.unlockedBuildings.includes(type),
			),
		}))
	...
	return (
		<div className="flex flex-col w-72 shrink-0 h-full factory-panel overflow-hidden">
			...
			<div className="factory-panel-header">
				...
				<div>
					<h2 className="font-bold text-[var(--factory-text-primary)]">
						Buildings
					</h2>
					<p className="text-[10px] text-[var(--factory-text-muted)]">
						Click to select, then place on grid
					</p>
				</div>
			</div>
										...
										<div className="flex items-start gap-3 relative z-10">
											<div
												className={cn(
													...
													isSelected &&
														"border-[var(--factory-amber)] shadow-[0_0_12px_var(--factory-amber-glow)]",
												)}
											>
												<Icon
													className={cn(
														...
														isSelected
															? "text-[var(--factory-amber)]"
															: "text-[var(--factory-text-secondary)]",
													)}
												/>
											</div>
			...
			<div className="p-4 border-t border-[var(--factory-border)] bg-[var(--factory-bg-deep)]">
				<p className="text-[11px] text-[var(--factory-text-muted)] leading-relaxed">
					<span className="text-[var(--factory-amber)] font-semibold">
						Left-click
					</span>{" "}
					on grid to place building.
					<br />
					<span className="text-[var(--factory-danger)] font-semibold">
						Right-click
					</span>{" "}
					to demolish.
				</p>
