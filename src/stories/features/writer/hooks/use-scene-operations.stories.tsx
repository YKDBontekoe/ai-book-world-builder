...
const SceneOperationsDemo = ({
  ...
  const [structure, setStructure] = useState(mockStructure);

  // Mock server actions by overriding the hook's dependencies if possible,
  // but since we can't easily inject mocked actions into the hook in Storybook without a provider or prop injection (which the hook doesn't support fully for actions),
  // we will rely on the fact that the hook imports actions directly.
  // For visual testing, we might see errors if the backend is not reachable,
  // but we can observe the UI state changes (Optimistic UI).

  // NOTE: In a real integration scenario, we would mock the network or the actions.
  // Here, we focus on the UI states exposed by the hook.
    ...
    <div className="p-8 space-y-6 max-w-2xl mx-auto border rounded-xl bg-card text-card-foreground">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="p-4 border rounded-lg space-y-2">
          ...
          >
            {deletedSceneIds.has(MOCK_SCENE_ID) ? "Deleted (Undo available)" : "Delete Scene 1"}
          </Button>
        </div>
      </div>
...
export const WithSimulatedError: Story = {
  args: {
    simulateError: true,
  },
};
