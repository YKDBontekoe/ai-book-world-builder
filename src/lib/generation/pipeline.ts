...
import { StepExecutionLogger } from "@/lib/generation/step-logger";
import { GenerationCallbacks } from "@/lib/generation/step-logger";
...
export class GenerationPipeline {
    ...
    constructor(private options: RunGenerationOptions) {
        this.logger = new StepExecutionLogger(options.generationId, options.callbacks);
    }
    ...
    async execute() {
        ...
        try {
            ...
            // 1. Fetch Data
            const projectData = await getFullProjectDataForGeneration({ projectId, userId });
    ...
    private async shouldPauseOrStop(generationId: string, log: (msg: string) => void): Promise<boolean> {
