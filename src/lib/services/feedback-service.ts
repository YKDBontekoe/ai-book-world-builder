...
import { feedback, type FeedbackType, type Feedback } from "@/lib/db/schema/feedback";
...
export async function processDailyFeedback() {
    const pending = await db.select().from(feedback).where(
    ...
    for (const group of object.groups) {
        try {
            ...
            console.error(`Failed to create session for group ${group.featureName}`, e);
        }
    }
