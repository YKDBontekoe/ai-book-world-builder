...
import { ConsistencyIssue, ConsistencyIssueType, ConsistencyIssueSeverity } from "@/lib/db/schema/issues";
...
const issueSchema = z.object({
    issues: z.array(z.object({
        ...
        sceneIds: z.array(z.string()).describe("IDs of scenes involved in this issue (if applicable)"),
    }))
});
...
export class ConsistencyService {
    async analyzeProject(projectId: string) {
        ...
        const draftedScenes = scenes.filter(s => s.status === "drafted" || s.status === "final" || s.status === "review");
        ...
        const sceneContext = scenesToAnalyze.map(s => `
Scene ID: ${s.id}
Title: ${s.title}
Content:
${s.content || "(No content)"}
        ...
        const { object } = await generateObject({
            ...
            schema: issueSchema,
            system: "You are a continuity editor for a novel. Your job is to find inconsistencies between the 'World Bible' (Entities) and the written 'Scenes'. Look for character description errors, plot holes, or tone shifts.",
            prompt: `
...
`,
        ...
        const newIssues: Partial<ConsistencyIssue>[] = object.issues.map(issue => ({
            projectId,
