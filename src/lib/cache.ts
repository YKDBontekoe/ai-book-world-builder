...
export async function invalidateCachePattern(pattern: string): Promise<void> {
	if (!redis) return;
	try {
		// Scan for keys matching pattern using @upstash/redis
		let cursor = 0;
		do {
			// Upstash `scan` returns a tuple: [newCursor: string, keys: string[]]
			...
			if (keys.length > 0) {
				// Upstash `del` accepts multiple arguments, not an array
