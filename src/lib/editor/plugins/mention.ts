...
export const mentionPlugin = (onMentionStateChange: (state: MentionState | null) => void) => new Plugin({
    key: mentionPluginKey,
    state: {
        init(): MentionState {
            return { active: false, range: null, query: "", index: 0 };
        },
        apply(tr: Transaction, value: MentionState, oldState: EditorState, newState: EditorState): MentionState {
            const { selection } = tr;
            ...
            if (match) {
                ...
                const to = $from.pos;
                return { active: true, range: { from, to }, query, index: value.index || 0 };
            }
    ...
    props: {
        handleKeyDown(view: EditorView, event: KeyboardEvent) {
             ...
             if (event.key === "ArrowUp") {
                 onMentionStateChange({ ...state, index: Math.max(0, state.index - 1) });
                 return true;
             }
             if (event.key === "Enter") {
