...
export async function getProjectStats(projectId: string): Promise<{ tokenStats: TokenStats; entityStats: EntityStats }> {
  // 1. Entity Stats
  ...
  const genUsage = await db
    ...
    .innerJoin(bookGeneration, eq(bookGenerationStep.generationId, bookGeneration.id))
    .where(and(
  ...
  for (const row of genUsage) {
      ...
      if (!tokenStats.byModel[modelId]) {
          tokenStats.byModel[modelId] = { cost: 0, inputTokens: 0, outputTokens: 0 };
      }
...
export async function getGlobalStats(userId: string): Promise<{ tokenStats: TokenStats; entityStats: EntityStats }> {
