...
function PureModelSelectorCompact({
        ...
        const { favoriteModels, recentModels, toggleFavorite, moveFavorite, recordRecentModel, error } =
                useModelPreferences();
        ...
        const filteredModels = useMemo(() => {
                ...
                return [...filtered].sort((a, b) => {
                        switch (sortOption) {
                                case "price-asc": {
                                        const priceA = a.pricing?.input ? getPricePerMillion(a.pricing.input) : 0;
                                        const priceB = b.pricing?.input ? getPricePerMillion(b.pricing.input) : 0;
        ...
        const selectedModel =
                availableModels.find((model) => model.id === optimisticModelId) ?? fallbackModel;

        const getModelItemValue = useCallback((modelId: string, prefix: string) => `${prefix}__${modelId}`, []);
        ...
        const currentSelectValue = useMemo(() => {
                ...
                const provider = availableModels.find((model) => model.id === optimisticModelId)?.provider;
                return getModelItemValue(optimisticModelId, provider ? `all-${provider}` : "all");
        }, [availableModels, favoriteModels, optimisticModelId, recentModels, getModelItemValue]);
        ...
        const handleMoveFavorite = useCallback(
                (event: React.MouseEvent | React.PointerEvent, modelId: string, direction: "up" | "down") => {
                        event.preventDefault();
                        event.stopPropagation();
        ...
        const renderModelCard = useCallback(
                                                ...
                                                <div className="flex items-center gap-1">
                                                        {keyPrefix === "fav" && (
                                                                <div className="flex items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
                                                                        <button
                                                                                type="button"
                                                                                onPointerDown={(event) => handleMoveFavorite(event, model.id, "up")}
                                                                        ...
                                                                        <button
                                                                                type="button"
                                                                                onPointerDown={(event) => handleMoveFavorite(event, model.id, "down")}
                                                                                className="rounded p-0.5 text-muted-foreground hover:bg-muted hover:text-foreground disabled:opacity-30"
                                                                                disabled={favoriteModels.indexOf(model.id) === favoriteModels.length - 1}
                                                                        >
                ...
                [favoriteModels, getModelItemValue, handleMoveFavorite, handleToggleFavorite, optimisticModelId],
        );

        return (
                <PromptInputModelSelect onValueChange={handleSelectModel} value={currentSelectValue}>
                        <Trigger asChild>
                        ...
                        >
                                ...
                                <div className="sticky bottom-0 border-t bg-muted/30 px-3 py-2 text-center">
                                        <span className="text-[10px] text-muted-foreground">
                                                {availableModels.length} models available • {favoriteModels.length} favorites
                                        </span>
                                </div>
                        </PromptInputModelSelectContent>
