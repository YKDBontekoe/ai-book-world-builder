...
import { Brain, Coins, HelpCircle, Image, Settings, Zap } from "lucide-react";
...
export function GenerationConfigPanel({
	...
	const writerModel: ChatModel | undefined = chatModels.find(
		(m) => m.id === settings.writerModelId,
	);
	const reviewerModel: ChatModel | undefined = chatModels.find(
		(m) => m.id === settings.reviewerModelId,
	);
	const writerCost =
		((estimatedWords * 1.3) / 1_000_000) *
				...
				>
					...
					<ModelSelection
						...
						onModelChange={(modelId) =>
							updateSetting("reviewerModelId", modelId)
						}
